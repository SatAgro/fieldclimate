...
class ClientBuilder:
    ...
    class _ConnectionBase:
        ...
        @abstractmethod
        def _modifyRequest(self, requestContents):
            pass
        ...
        async def _make_request(self, method, route, body=None):
            ...
            self._modifyRequest(requestContents)
            
            args = [self.apiURI+'/'+requestContents.route]
            ...
            result = await reqMethod(*args, **kwargs)
...
async def trivialTest():
    async with ClientBuilder.HMAC('', '') as client:
        user = await client.list_of_user_devices()
        print(user)
